...
import { X, Image as ImageIcon, Sliders, Edit3, Save } from 'lucide-react';
...
interface EditRoomModalProps {
  ...
  onClose: () => void;
  onSubmit: (id: string, updatedData: Partial<AccommodationRoom>) => void;
}

export const EditRoomModal: React.FC<EditRoomModalProps> = ({ room, onClose, onSubmit }) => {
  const [activeStudioTab, setActiveStudioTab] = useState<'specs' | 'media'>('specs');

  const [name, setName] = useState(room.name || '');
  const [category, setCategory] = useState(room.category || 'Ocean Villa');
  const [tagline, setTagline] = useState(room.tagline || 'Executive Luxury Suite with Sunset Plunge Pool');
  const [location, setLocation] = useState(room.location || 'Victoria Island Promenade, Lagos');
  const [pricePerNight, setPricePerNight] = useState<number>(room.pricePerNight || 450);
  const [priceNairaPerNight, setPriceNairaPerNight] = useState<number>(room.priceNairaPerNight || 720000);
  const [bedrooms, setBedrooms] = useState<number>(room.bedrooms || 2);
  const [bathrooms, setBathrooms] = useState<number>(room.bathrooms || 2);
  const [maxGuests, setMaxGuests] = useState<number>(room.maxGuests || 4);
  const [floor, setFloor] = useState<number>(room.floor || 3);
  const [squareMeters, setSquareMeters] = useState<number>(room.squareMeters || 95);
  const [bedType, setBedType] = useState(room.bedType || 'Super King Bed');
  const [heroImage, setHeroImage] = useState(room.heroImage || 'https://images.unsplash.com/photo-1618773928121-c32242e63f39?auto=format&fit=crop&q=85&w=1600&h=900');
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3.5 border-b border-zinc-100">
          <div>
            <div className="flex items-center gap-2">
              <span className="px-2.5 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider bg-indigo-50 text-indigo-900 border border-indigo-100 flex items-center gap-1">
                <Edit3 className="w-3 h-3 text-indigo-600" />
                Edit Executive Suite Studio
              </span>
            </div>
            <h2 className="text-base sm:text-lg font-bold text-zinc-900 mt-0.5">Edit {room.name}</h2>
          </div>
          ...
          <div className="lg:col-span-7 space-y-4">
            ...
            <div className="flex items-center justify-end gap-2.5 pt-4 border-t border-zinc-100">
              ...
              <button type="submit" className="px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold shadow-xs cursor-pointer flex items-center gap-1.5">
                <Save className="w-4 h-4" /> Save Suite Changes
              </button>
            </div>
          </div>
