...
import { X, KeyRound, CreditCard, Banknote, Building, User, Phone, Mail, Sparkles } from 'lucide-react';
      ...
      <div className="bg-white rounded-xl max-w-2xl w-full max-h-[92vh] overflow-y-auto shadow-2xl border border-zinc-200/80 custom-scrollbar">
        <div className="p-5 border-b border-zinc-100 flex items-center justify-between sticky top-0 bg-white z-10">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-emerald-600 flex items-center gap-1"><Sparkles className="w-3.5 h-3.5" /> Instant Check-In Ready</span>
            </div>
        ...
        {!confirmed ? (
          <form onSubmit={handleConfirmBooking} className="p-5 space-y-5">
            <div className="p-3.5 rounded-lg bg-zinc-50 border flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <img src={room.heroImage} alt={room.name} className="w-14 h-14 rounded-lg object-cover" />
                <div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <div className="relative"><User className="w-4 h-4 text-zinc-400 absolute left-3 top-1/2 -translate-y-1/2" /><input type="text" required value={guestName} onChange={(e) => setGuestName(e.target.value)} className="w-full pl-9 pr-3 py-2 text-xs border rounded-lg" /></div>
                </div>
                <div>
                  <label className="block text-xs font-medium text-zinc-700 mb-1">Phone Number</label>
                  <div className="relative"><Phone className="w-4 h-4 text-zinc-400 absolute left-3 top-1/2 -translate-y-1/2" /><input type="text" required value={guestPhone} onChange={(e) => setGuestPhone(e.target.value)} className="w-full pl-9 pr-3 py-2 text-xs border rounded-lg" /></div>
                </div>
                <div>
                  <label className="block text-xs font-medium text-zinc-700 mb-1">Email Address</label>
                  <div className="relative"><Mail className="w-4 h-4 text-zinc-400 absolute left-3 top-1/2 -translate-y-1/2" /><input type="email" required value={guestEmail} onChange={(e) => setGuestEmail(e.target.value)} className="w-full pl-9 pr-3 py-2 text-xs border rounded-lg" /></div>
                </div>
                <div>
                  <label className="block text-xs font-medium text-zinc-700 mb-1">ID / Passport Number</label>
                  <input type="text" required value={idPassport} onChange={(e) => setIdPassport(e.target.value)} className="w-full px-3 py-2 text-xs border rounded-lg" />
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3 pt-2 border-t">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  ...
                  <select value={selectedRoomNumber} onChange={(e) => setSelectedRoomNumber(e.target.value)} className="w-full px-3 py-2 text-xs border rounded-lg">
                ...
                <div>
                  ...
                  <select value={vipTier} onChange={(e: any) => setVipTier(e.target.value)} className="w-full px-3 py-2 text-xs border rounded-lg">
                ...
                <div>
                  ...
                  <div className="flex gap-1.5">
                    <button type="button" onClick={() => setCurrency('NGN')} className={`flex-1 py-1.5 text-xs rounded-lg font-bold ${currency === 'NGN' ? 'bg-ink text-white' : 'bg-zinc-100'}`}>NGN</button>
                    <button type="button" onClick={() => setCurrency('USD')} className={`flex-1 py-1.5 text-xs rounded-lg font-bold ${currency === 'USD' ? 'bg-ink text-white' : 'bg-zinc-100'}`}>USD</button>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2 pt-2 border-t">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {[
                  ...
                  return (
                    <button key={item.id} type="button" onClick={() => setPaymentMethod(item.id as any)} className={`p-2.5 rounded-lg border text-left flex flex-col cursor-pointer ${paymentMethod === item.id ? 'border-ink bg-[#EEF2FF] text-ink font-bold' : 'border-zinc-200 bg-white'}`}>
            ...
            <div>
              ...
              <textarea rows={2} value={specialNote} onChange={(e) => setSpecialNote(e.target.value)} className="w-full px-3 py-2 text-xs border rounded-lg" />
            </div>

            <div className="flex items-center justify-end gap-2.5 pt-3 border-t">
              <button type="button" onClick={onClose} className="px-4 py-2 border rounded-lg text-xs font-semibold">Cancel</button>
              <button type="submit" disabled={isProcessing} className="flex items-center gap-1.5 px-5 py-2.5 rounded-lg bg-ink text-white text-xs font-bold">
