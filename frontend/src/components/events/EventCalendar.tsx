...
export function EventCalendar({ events }: EventCalendarProps) {
  ...
  const eventOccurrences = pastEvents.flatMap(event => {
    if (event.isRecurring && event.recurrenceRule) {
      return expandRecurringEvent(event, `${viewYear}-01-01`, `${viewYear}-12-31`)
    }
    const eventYear = new Date(event.startDate).getFullYear()
    if (eventYear === viewYear) {
      return [{ ...event, occurrenceDate: event.startDate, isRecurrenceInstance: false, bookedCount: 0 }]
    }
    return []
  })

  const eventsByDate = eventOccurrences.reduce((acc, occ) => {
    ...
    <div className="space-y-6">
      ...
      >
        <div className="relative" style={{ width: totalWidth, height: 120 }}>
          ...
          {viewYear === new Date().getFullYear() && (
            <div className="absolute top-0 bottom-0 w-px bg-bronze-deep animate-pulse" style={{ left: getDayPosition(new Date().toISOString().split('T')[0]) * dayWidth }} />
          )}
          ...
          {Object.entries(eventsByDate).map(([dateStr, dayEvents]) => {
            ...
            return (
              <div
                ...
                onMouseEnter={() => setSelectedEvent(dayEvents[0])}
      ...
      <div className="flex flex-wrap items-center gap-4 px-4 text-sm text-zinc-600">
        ...
        <span className="ml-auto flex items-center gap-1.5 text-[11px]">
          <span className="w-3 h-3 rounded bg-bronze-deep animate-pulse" title="Today" />
          Today
        </span>
      </div>
          ...
          <div className="relative w-full sm:w-96 bg-white rounded-t-2xl sm:rounded-tl-2xl sm:rounded-bl-2xl shadow-2xl p-6 max-h-[90vh] overflow-y-auto z-10 animate-in slide-in-from-right">
            ...
            <div className="space-y-4">
              ...
              <Link
                ...
                className="block mt-4 text-center px-4 py-2 bg-bronze-deep text-white rounded-lg font-medium hover:bg-bronze-deep/90 transition-colors"
                onClick={() => setSelectedEvent(null)}
              >
                View Event Details
              </Link>
